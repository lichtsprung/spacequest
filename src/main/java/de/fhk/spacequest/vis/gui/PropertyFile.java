...
public abstract class PropertyFile {
    ...
    public static List<GUIComponent> createGUIElements(String pathToFile, GUI gui) throws Exception {
        ...
        Document template = builder.parse(pathToFile);
        ...
        for (int index = 0; index < elementNodes.getLength(); index++) {
            ...
            for (; typeNumber < elementTypes.length; typeNumber++) {
                if (type.equals(elementTypes[typeNumber])) break;
            }
            
            switch(typeNumber) {
                case (0): components.add(createButtonFromNode(node,gui));
                            //System.out.println("Button");
                    break;
                case (1): components.add(createClockFromNode(node,gui));
                            //System.out.println("Clock");
                    break;
                case (2): components.add(createSliderFromNode(node,gui));
                            //System.out.println("Slider");
                    break;
                case (3): createFactoryFromNode(node, components, gui);
    ...
    private static GUIComponent createButtonFromNode(Element node, GUI gui) {
        ...
        for (; subtypeNumber < buttonSubtypes.length; subtypeNumber++) {
            if (subtype.equals(buttonSubtypes[subtypeNumber])) break;
        }

        try {
            switch(subtypeNumber) {
                case (0): component = new StartToggleButton(Integer.parseInt(node.getAttribute("x")), Integer.parseInt(node.getAttribute("y")), gui, gui.IniFile.get(3), gui.IniFile.get(4));
                    break;
                case (1): component = new PresetToggleButton(Integer.parseInt(node.getAttribute("x")), Integer.parseInt(node.getAttribute("y")), gui, gui.IniFile.get(3), gui.IniFile.get(4));
                    break;
                default :
                    break;
            }
    ...
    private static GUIComponent createClockFromNode(Element node, GUI gui) {
        ...
        for (; subtypeNumber < clockSubtypes.length; subtypeNumber++) {
            if (subtype.equals(clockSubtypes[subtypeNumber])) break;
        }

        try {
            switch(subtypeNumber) {
                case (0): component = new AnalogClock(Integer.parseInt(node.getAttribute("x")), Integer.parseInt(node.getAttribute("y")), Integer.parseInt(node.getAttribute("size")), gui, gui.IniFile.get(2));
                    break;
                case (1): component = new DigitalClock(Integer.parseInt(node.getAttribute("x")), Integer.parseInt(node.getAttribute("y")), gui, gui.IniFile.get(2));
                    break;
                default :
                    break;
            }
    ...
    private static GUIComponent createSliderFromNode(Element node, GUI gui) {
        ...
        for (; subtypeNumber < sliderSubtypes.length; subtypeNumber++) {
            if (subtype.equals(sliderSubtypes[subtypeNumber])) break;
        }

        try {
            switch(subtypeNumber) {
                case (0): component = new ZoomSlider(Integer.parseInt(node.getAttribute("x")), Integer.parseInt(node.getAttribute("y")), Float.parseFloat(node.getAttribute("min")), Float.parseFloat(node.getAttribute("max")), Integer.parseInt(node.getAttribute("size")), gui, gui.IniFile.get(7));
                    break;
                default :
                    break;
            }
